...
class GenerateDialog(val project: Project, val file: VirtualFile, val notificationManager: GenerationNotificationManager) : DialogWrapper(project) {
    ...
    init {
        outputBrowse.addBrowseFolderListener(
                Message of "dialog.generate.output-browse.title",
                null,
                project,
    ...
    private fun createTypeMappingsPanel() {
        typeMappingsPanel = KeyValuePropertiesPanel(
                Message of "panel.type-mappings.key-column",
                Message of "panel.type-mappings.value-column"
        )
    ...
    override fun doValidate(): ValidationInfo? {
        // valid Swagger file
        io.swagger.parser.SwaggerParser().read(file.path) ?: return ValidationInfo("Swagger file is invalid.", null)
